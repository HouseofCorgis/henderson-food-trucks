...
const icons: Record<string, string> = { 'BBQ': '🍖', 'Mexican': '🌮', 'Burgers': '🍔', 'Vietnamese': '🍜', 'American': '🧀', 'Desserts': '🍦', 'Southern': '🍗', 'Mediterranean': '🥙', 'Pizza': '🍕', 'Vegan': '🥗', 'Coffee': '☕', 'Seafood': '🦐', 'Korean': '🍚', 'Indian': '🍛' };
...
export default function TruckCard({ truck, schedule, venues }: { truck: Truck; schedule: Schedule[]; venues: Venue[] }) {
  ...
  const handleFacebookClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    e.preventDefault();
    analytics.socialClick(truck.id, 'facebook');
    window.open(truck.facebook, '_blank');
  };

  const handleInstagramClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    e.preventDefault();
    analytics.socialClick(truck.id, 'instagram');
    window.open(truck.instagram, '_blank');
  };

  const handleCardClick = () => {
    analytics.profileView(truck.id);
  };

  return (
    <Link href={`/trucks/${truck.id}`} onClick={handleCardClick} className="block">
      <div className="bg-white rounded-2xl shadow-sm hover:shadow-lg transition-all overflow-hidden cursor-pointer">
